...
public class Bullet : MonoBehaviour
{
    ...
    private Vector3 initialPosition; // 총알의 초기 위치 저장
...
public class BulletMovement : MonoBehaviour
{
    ...
    void Update()
    {
        // 총알이 Z축을 기준으로 3 이상 이동하면 총알 파괴
        if (Mathf.Abs(transform.position.z - initialPosition.z) >= 3f)
        {
            Destroy(gameObject);
            Debug.Log("총알이 Z축 3만큼 이동하여 파괴됨");
        } // 총알이 Y축을 기준으로 3 이상 이동하면 총알 파괴
        else if(Mathf.Abs(transform.position.x - initialPosition.x) >= 3f)
        {

            Destroy(gameObject);
            Debug.Log("총알이 X축 3만큼 이동하여 파괴됨");
        }
    }
}